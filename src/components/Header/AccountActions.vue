<template>
  <div class="account-actions" @mouseover="open" @mouseleave="close">
    <button @click="toggle" class="account-actions__btn">
      ...
      <!-- <UserAvatar :avatarUrl="null" class="account-actions__avatar" /> -->
      <svg
    ...
    <ul v-show="isOpen" class="account-actions__list">
      <li class="account-actions__item">
        <router-link :to="{ name: 'orders-page' }" class="account-actions__link"
  ...
  methods: {
    ...
    async handleLogout() {
      try {
        ...
        this.$notify({ type: 'error', header: 'Логаут не удался', text: error.message });
      }
    },
  },
};
